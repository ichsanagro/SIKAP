<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Ubah Nilai Seminar Mahasiswa') }}
        </h2>
    </x-slot>
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="mb-6">
                        <h3 class="text-lg font-semibold text-gray-900">
                            {{ $examinerSeminarScore->application->student->name }} - {{ $examinerSeminarScore->application->title }}
                        </h3>
                    </div>

                    <form action="{{ route('supervisor.seminar.scores.update', $examinerSeminarScore) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                            <div>
                                ...
                                <input type="number" name="laporan" id="laporan" min="0" max="100" required
                                       value="{{ old('laporan', $examinerSeminarScore->laporan) }}"
                            ...
                            <div>
                                ...
                                <input type="number" name="presentasi" id="presentasi" min="0" max="100" required
                                       value="{{ old('presentasi', $examinerSeminarScore->presentasi) }}"
                                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                       placeholder="0-100">
                                @error('presentasi')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <input type="number" name="sikap" id="sikap" min="0" max="100" required
                                       value="{{ old('sikap', $examinerSeminarScore->sikap) }}"
                                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                       placeholder="0-100">
                                @error('sikap')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="catatan" id="catatan" rows="4"
                                      class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                      placeholder="Tambahkan catatan penilaian...">{{ old('catatan', $examinerSeminarScore->catatan) }}</textarea>
                            @error('catatan')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 mb-6">
                            ...
                            <div class="grid grid-cols-3 gap-4 text-sm">
                                <div>
                                    ...
                                    <span id="total-score" class="font-medium text-gray-900">{{ $examinerSeminarScore->total_skor }}</span>
                                </div>
                                <div>
                                    <span class="text-gray-600">Rata-rata:</span>
                                    <span id="average-score" class="font-medium text-gray-900">{{ number_format($examinerSeminarScore->rata_rata, 2) }}</span>
                                </div>
                                <div>
                                    <span class="text-gray-600">Nilai Huruf:</span>
                                    <span id="grade" class="font-medium text-gray-900">{{ $examinerSeminarScore->nilai_huruf }}</span>
                                </div>
                            </div>
                        </div>
