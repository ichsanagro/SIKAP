                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <div class="mb-6">
                        <h3 class="text-lg font-semibold text-gray-900">
                            {{ $supervisorScore->application->student->name }} - {{ $supervisorScore->application->title }}
                        </h3>
                    </div>

                    <form action="{{ route('supervisor.scores.update', $supervisorScore) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                            ...
                            <div>
                                ...
                                <input type="number" name="presentation" id="presentation" min="0" max="100" required
                                       value="{{ old('presentation', $supervisorScore->presentation) }}"
                                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                       placeholder="0-100">
                                @error('presentation')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <input type="number" name="attitude" id="attitude" min="0" max="100" required
                                       value="{{ old('attitude', $supervisorScore->attitude) }}"
                                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                       placeholder="0-100">
                                @error('attitude')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="notes" id="notes" rows="4"
                                      class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                                      placeholder="Tambahkan catatan penilaian...">{{ old('notes', $supervisorScore->notes) }}</textarea>
                            @error('notes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 mb-6">
                            ...
                            <div class="grid grid-cols-2 gap-4 text-sm">
                                <div>
                                    ...
                                    <span id="final-score" class="font-medium text-gray-900">{{ number_format($supervisorScore->final_score, 2) }}</span>
                                </div>
                                <div>
                                    <span class="text-gray-600">Grade:</span>
                                    <span id="final-grade" class="font-medium text-gray-900">{{ $supervisorScore->grade }}</span>
                                </div>
                            </div>
                        </div>
