...
<div class="max-w-2xl mx-auto p-6">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Ubah Instansi</h1>
        <p class="text-gray-600 mt-2">Perbarui data instansi {{ $company->name }}.</p>
    </div>

    <div class="bg-white rounded-lg shadow p-6">
        <form method="POST" action="{{ route('admin-prodi.companies.update', $company) }}">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <textarea name="address" id="address" rows="3"
                          class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('address', $company->address) }}</textarea>
                @error('address')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="contact_phone" id="contact_phone" value="{{ old('contact_phone', $company->contact_phone) }}"
                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                @error('contact_phone')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            {{-- <div class="mb-4">
                ...
                <select name="batch" id="batch" required
                    ...
                    <option value="1" {{ old('batch', $company->batch) == '1' ? 'selected' : '' }}>Opsi 1</option>
                    <option value="2" {{ old('batch', $company->batch) == '2' ? 'selected' : '' }}>Opsi 2</option>
                </select>
                @error('batch')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                ...
                <input type="number" name="quota" id="quota" value="{{ old('quota', $company->quota) }}" min="1" required
